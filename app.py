...
def predict():
    ...
    if query_sentiments[0][0]>0.5:
            sentiment = f'Positive Sentiment  with probable rating as: {np.round(query_sentiments[0][0]*10,1)}'
            img_filename = os.path.join(app.config['UPLOAD_FOLDER'], 'Smiling_Emoji.png')

    else:
            sentiment = f'Negative Sentiment with probable  rating as: {np.round(query_sentiments[0][0]*10,1)}'
            img_filename = os.path.join(app.config['UPLOAD_FOLDER'], 'Sad_Emoji.png')
    
    return render_template('index.html', text=sentiment, sentiment=sentiment, image=img_filename)
